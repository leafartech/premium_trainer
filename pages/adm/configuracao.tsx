...
const Configuracao = () => {
  ...
  async function formSubmited(data: any /*SubmitHandler<>*/) {
    await axios
      .post("/api/config", { data, user: userEmail.email })
      .then((res) => {
        if (typeof window !== "undefined") {
          localStorage.setItem("train-system", JSON.stringify(res.data))
          setCurrentHours(res.data.schedule.avaibleHours)
    ...
    <>
      {access && (
        <>
          ...
          <main className="px-4">
            <div className="py-4">
              <h1 className="text-brandWhite relative flex items-center gap-2 text-3xl mb-2 font-bold">
                Horários
                {message && <span className="bg-emerald-500 text-xs text-white rounded-md p-2">{message}</span>}
              </h1>
              <p className="text-zinc-400 text-sm">
                Selecione os horários que você pode atender. Esses horários
                serão disponibilizados os clientes para realizarem o
                agendamento.
              </p>
              <p className="text-zinc-400 text-sm border rounded-lg mt-4 p-4 border-dashed border-gray-400">
                O sistema foi criado para cada consulta durar na média de 1
                hora.
              </p>
            </div>
              ...
              <div className="grid grid-cols-4 gap-2">
                {arrHour.map((hour, index) => (
                  ...
                  >
                    ...
                    />
                    <p className="text-brandWhite">{hour}</p>
                  </label>
                ))}
              </div>
              <button
                type="submit"
                className="py-2 bg-emerald-500 text-brandWhite font-semibold text-center w-full rounded-md mt-6"
              >
                Atualizar horários
              </button>
            </form>
            <div className="text-zinc-300 flex flex-col items-start gap-3 mt-6">
              {typeof currentHours !== "undefined" &&
                currentHours.length > 0 ? (
                <>
                  <h4 className="text-brandWhite text-lg font-medium">
                    Horários selecionados anteriormente
                  </h4>
                ...
                <h4 className="text-brandWhite text-lg font-medium">
                  Você ainda não selecionou nenhum horário.
                </h4>
              )}
            </div>
          </main>
