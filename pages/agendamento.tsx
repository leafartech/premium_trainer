...
const Agendamento = () => {
    ...
    const [avaibleDay, setAvaibleDay] = useState<boolean>(false)
    const [avaibleHour, setAvaibleHour] = useState<boolean>(false)
    ...
    useEffect(() => {
        axios.get('/api/avaibleHours').then(res => {
                ...
                <form className="w-96 flex flex-col gap-6" onSubmit={e => formSubmited(e)}>
                        ...
                        <select value={month} onChange={e => {
                            ...
                            {months.map((month, index) => (
                                currentMonth - 1 > index ? <>
                                    <option key={index} className="bg-brandBlack" disabled value={`${month}-${index}`}>{month}</option>
                                </> : (
                                    <option key={index} className="bg-brandBlack" value={`${month}-${index}`}>{month}</option>
                                )
                            ))}
                        </select>
                    </div>
                    <div className="flex flex-col gap-1">
                        ...
                        {avaibleDay ?
                                ...
                                {!isNaN(parseInt(month[month.length - 2])) ? //Verificação para ver se mês possui mais de 2 dígitos (Outubro, Novembro e Dezembro)
                                    <>
                                        {arraySupport[parseInt(`${parseInt(month[month.length - 2])}${parseInt(month[month.length - 1])}`)].map((day, index) => (
                                            <option key={index} className="bg-brandBlack" value={day}>{day}</option>
                                        ))}
                                    </>
                                    :
                                    <>
                            ...
                            :
                            <select disabled className="outline-none text-zinc-400 py-2 px-2 bg-transparent border border-gray-600 rounded-lg">
                                <option className="bg-brandBlack">Primeiro, selecione o mês</option>
                            </select>
                        }
                    </div>
                    <div className="flex flex-col gap-1">
                        ...
                        {avaibleHour ?
                            <select value={day} onChange={e => setHour(e.target.value)} className="outline-none text-zinc-300 py-2 px-2 bg-transparent border border-gray-600 rounded-lg">
                                {avaibleHour ?
                                    <>
                                        {userSchedule?.avaibleHours.map((hour, index) => (
                                            <>
                                                {typeof userSchedule?.months[`${month.split('-')[0]}`][day][hour] !== 'undefined' ?
                                                    <option key={index} className="bg-brandBlack" value={hour} disabled>{hour}:00</option>
                                                    :
                                                    <option key={index} className="bg-brandBlack" value={hour}>{hour}:00</option>
                                                }
                                            </>
                                        ))}
                                    </>
                            ...
                            :
                            <select disabled className="outline-none text-zinc-400 py-2 px-2 bg-transparent border border-gray-600 rounded-lg">
                                <option className="bg-brandBlack">Primeiro, selecione o dia</option>
                            </select>
                        }
                    </div>
                    {disabled ?
